  ...
  const renderContent = () => {
    ...
    return (
      <div className="h-full w-full flex overflow-hidden bg-zinc-100 relative">
        {/* Left Panel */}
        <div
          className={`h-full shadow-sm z-10 transition-all duration-300 ease-in-out flex shrink-0 ${isLeftPanelOpen ? 'w-1/4 min-w-[250px] max-w-[350px]' : 'w-0 overflow-hidden'
            }`}
        >
        ...
        <div className="flex-1 h-full shadow-xl z-20 relative flex flex-col min-w-0">
          ...
          <Editor
            ...
            onUpdateBookLayout={(layoutId) => setBook({ ...book, layoutId })}
            onUpdateBook={setBook}
        ...
        {/* Right Panel */}
        <div
          className={`h-full shadow-sm z-10 transition-all duration-300 ease-in-out flex shrink-0 ${isRightPanelOpen ? 'w-1/4 min-w-[300px] max-w-[400px]' : 'w-0 overflow-hidden'
            }`}
        >
        ...
        <>
          ...
          <header className="h-14 bg-zinc-900 text-white flex items-center justify-between px-6 shrink-0 z-50 border-b border-zinc-800">
            <div className="flex items-center font-semibold text-lg tracking-tight">
              <img src="../assets/fav.svg" alt="Exitus Material Logo" className="w-6 h-6 mr-2" />
              ExitusMaterial
            </div>
            <button
              onClick={handleReset}
